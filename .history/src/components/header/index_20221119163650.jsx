...
export default function Header() {
  ...
  const onChangeInput = (e) => {
    console.log("输入框发生变化::,",e.target.value)
    setInputValue(e.target.value)
    
  }

  useEffect(() => {
    // PubSub.subscribe("changeInput", (name, val) => {
    //   console.log("发生变化", val,inputContent)
    //   setInputValue(val)
    // })
  }, [])
  ...
  const OperationsSlot = {
    ...
    right:
    <input type="text" key={Math.random()} onChange={onChangeInput} value={inputContent} />
      // <Search
  ...
  return (
    ...
    <div className={style.box}>
      <div className={style.headers}>
        ...
        <input className={style.searchInput} type="text" onChange={onChangeInput} value={inputContent} />
