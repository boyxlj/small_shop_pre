...
export default function PayDialog () {
  ...
  const paySuccess = ()=>{
    ...
    timer = setTimeout(async()=>{
      ...
      dispatch(setSuccessTip({
        ...
        // if(location.href.includes("/order/orderDetails?")){
        //   console.log("是当前路径")
        // }
    },4000)
    setTimer(timer)
  ...
  return (
      <Modal title="请扫码支付" closable={false} style={{textAlign:"center"}}  keyboard={false} maskClosable={false} footer={null} visible={payDialog} onCancel={handleCancel}>
       <div className={style.payImg}>
        <img src={palImg} alt="" />
       </div>
       <div className={style.btns}>
        <Button onClick={handleCancel} className={style.cancel}>取消支付</Button>
        <Button type='danger' onClick={paySuccess} loading={payLoading} className={style.success}>{payLoading?"支付校验中...":"我已支付"}</Button>
       </div>
      </Modal>
  );
};
